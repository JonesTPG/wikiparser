...
class Database(object):
  ...
  def save_result(self, source_id, source_title, target_id, target_title, distance, path):
    ...
    self.searches_conn.execute("INSERT INTO result VALUES(?,?,?,?,?,?,?);", (str(self.serial), str(source_id), str(source_title), str(target_id), str(target_title), str(distance), str(path),))
  ...
  def clean_results(self):
    self.searches_conn.execute("DELETE FROM result;")
    self.searches_conn.commit()